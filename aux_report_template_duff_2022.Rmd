...
rec_graph1<-ggplot(recommend_data, aes(x=recommend))+
  geom_bar(aes(y= (..count..)/sum(..count..)), fill = "deepskyblue1")+
...
rec_graph2<-ggplot(recommend_data, aes(x=recommend, fill=net_role))+
  ...
  labs (
    ...
    y="Percent of respondents",
    x="Network recommendation (1=Certainly not, 10=Certainly)")+
  scale_fill_manual(values=c("navy", "deepskyblue1"))+
  theme_classic()+  
  theme(legend.position="bottom",
        legend.background=element_rect(fill="gray100", color="black"),
        legend.title=element_blank())

rec_graph3<-ggplot(recommend_data, aes(x=recommend))+
...
rec_graph4<-ggplot(recommend_data, aes(x=recommend, group=net_role, fill=net_role))+
...
On average, your network members 
\newpage
