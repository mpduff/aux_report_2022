  ...
  officedown::rdocx_document:
    ...
    plots:
      ...
      caption:
        ...
        sep: ': '
params:
  network_name: "Denver Public Schools: The College Ready On Track Network"
  leaders_engage: FALSE
  leaders_networkcomms: FALSE
  nre_useful: TRUE
  nre_intensity: FALSE
  nre_expectations: TRUE
  ic_equity: FALSE
  ic_trust: FALSE
  sa_state: FALSE
  network_2021: DPS
...
network_data_2022<- read_csv("DPS-cleaned and deidentified.csv")
network_data_2021<- read_csv("aux_playset.csv")
network_roster_2021<- read_csv("roster_Denver Public Schools.csv")
network_roster_2022<- read_csv("roster_dps_2022.csv")
```
...
network_data_2022$net_role<-factor(network_data_2022$net_role,
levels=c(111,112,113,114),
labels=c("Hub members responsible for organization and leading the network", 
         "Central office staff supporting the network", 
         "School team leads", 
         "School site improvement team members"))

role_group_data<-network_data_2022%>%
  drop_na(net_role)%>%
  mutate(Total=length(net_role))
tab_role_group<-role_group_data%>%
  group_by(net_role)%>%
  summarize(Freq=n())%>%
...
network_data_2022<-network_data_2022%>%
  mutate(ethnicity_v3_1=ifelse(is.na(ethnicity_v3_1), 0, ethnicity_v3_1),
         ethnicity_v3_2=ifelse(is.na(ethnicity_v3_2), 0, ethnicity_v3_2),
         ethnicity_v3_3=ifelse(is.na(ethnicity_v3_3), 0, ethnicity_v3_3),
         ethnicity_v3_4=ifelse(is.na(ethnicity_v3_4), 0, ethnicity_v3_4),
         ethnicity_v3_5=ifelse(is.na(ethnicity_v3_5), 0, ethnicity_v3_5),
         ethnicity_v3_6=ifelse(is.na(ethnicity_v3_6), 0, ethnicity_v3_6),
         ethnicity_v3_7=ifelse(is.na(ethnicity_v3_7), 0, ethnicity_v3_7))%>%
...
network_data_2022<-network_data_2022%>%
  ...
  mutate(ethnicity_v3=sum(ethnicity_v3_1:ethnicity_v3_7))
network_data_2022$race_ethnicity<-as.factor(network_data_2022$race_ethnicity)

network_data_2022$race_ethnicity<-fct_relevel(network_data_2022$race_ethnicity,
                                         "Prefer not to say", "Additional racial/ethnic identity", "Multiple racial/ethnic identities", "White", "Native American/ First Nation", "Asian/ Hawaiian/ Pacific Islander", "Hispanic/ Latino/ Latina/ Latinx", "Black/ African American")

network_data_2022$race_ethnicity

race_data<-network_data_2022%>%
...
network_data_2022$gender_v3
network_data_2022$gender_v3<-factor(network_data_2022$gender_v3,
levels=c(1,2,3,4,5),
labels=c("Man", "Woman", "Gender queer/Gender non-conforming", "Identity not mentioned", "Prefer not to say"))
gender_data<-network_data_2022%>%
  drop_na(gender_v3)%>%
...
```{r prepare_knowleaders}
network_data_2022$nl_background_1<-factor(network_data_2022$nl_background_1,
                                          levels=c(1,2,3),
                                          labels=c("Yes, I know who the network leaders are",
                                                   "I think I know who the network leaders are, but I am not sure",
                                                   "I do not know who the network leaders are"))
know_leaders_data<-network_data_2022%>%
  drop_na(nl_background_1)%>%
  mutate(Total=length(nl_background_1))

know_leaders_data

tab_know_leaders<-know_leaders_data%%
  group_by(nl_background_1)%>%
  summarize(Freq=n())%>%
  mutate(Prop=Freq/sum(Freq))%>%
  mutate(Pct=percent(Prop, accuracy=1))
```

```{r prepare_nlgroupcom_stackedbars, include=FALSE, eval=params$leaders_networkcomms}
groupcom_data<-network_data_2022%>%
...
```{r prepare_time_bars, include=FALSE, eval=params$nre_intensity}
network_data_2022$so_part_t<-factor(network_data_2022$so_part_t,
                                    levels=c(1,2,3,4,5,6,7),
                                    labels=c("0-15 minutes", "16-30 minutes", "31-45 minutes", "46-60 minutes", "61-75 minutes", "76-90 minutes", "more than 90 minutes"))
time_data<-network_data_2022%>%
  drop_na(so_part_t)%>%
...
```{r prepare_expect_bars, include=FALSE, eval=params$nre_expectations}
network_data_2022$so_part_expec<-factor(network_data_2022$so_part_expec,
                                    levels=c(1,2,3,4,5),
...
```{r prepare_util_stackedbars, include=FALSE, eval=params$nre_useful}
useful_data<-network_data_2022%>%
...
recommend_data<-network_data_2022%>%
  drop_na(recommend_1)%>%
  mutate(Total=length(recommend_1))%>%
  mutate(Sum=sum(recommend_1))
  
total_recommend<-length(recommend_data$recommend_1)
sum_recommend<-sum(recommend_data$recommend_1)
mean_recommend<-round(sum_recommend/total_recommend, 2)
tab_recommend<-recommend_data%>%
  group_by(net_role, recommend_1)%>%
  summarize(Freq=n())%>%
  mutate(Prop=Freq/sum(Freq))%>%
  mutate(Pct=(percent(Prop, accuracy=1)))
mean_recommend
...
network_data_2022$part_team_meetings_1
network_data_2022$part_team_meetings_1<-factor(network_data_2022$part_team_meetings_1,
                  levels=c(1,2),
                  labels=c("Yes", "No"))
team_meet<-network_data_2022%>%
...
network_data_2022$part_team_meet_32021<-factor(network_data_2022$part_team_meet_32021,
                                                        levels=c(1,2,3,4,5),
                                                        labels=c("Never", "Every 3 months or less", "Every other month", "Once a month", "More than once a month"))
meet_freq_data<-network_data_2022%>%
...
network_data_2022$part_team_meetings_2<-factor(network_data_2022$part_team_meetings_2,
                                                    levels=c(1,2,3,4,5),
                                                    labels=c("Never", "Rarely", "Some meetings", "Almost all meetings", "All meetings"))
meet_part_data<-network_data_2022%>%
...
network_data_2022$is_inqcyclechal_y_n<-factor(network_data_2022$is_inqcyclechal_y_n,
levels=c(1,2,3),
labels=c("Yes", "No", "I'm not sure"))
inqyn_data<-network_data_2022%>%
...
network_data_2022$sus_begun_planning<-factor(network_data_2022$sus_begun_planning,
levels=c(1,2,3),
labels=c("Yes", "No", "Unsure"))
sus_plan_data<-network_data_2022%>%
...
specific_network_data_2021<-network_data_2021%>%
  filter(network==params$network_2021)
...
r22<-length(network_data_2022$respondent_id)
r22
members22<-length(network_roster_2022$firstname)
members22
rr22<-round((r22/members22)*100)
rr22
...
Overall, your network’s response rate was **`r rr22`%**. This is `r if(rr_diff>=3){"an improvement over"}` `r if(-3<=rr_diff& rr_diff<=3){"consistent with"}` `r if(rr_diff<=-3){"below"}` last year’s response rate, which was `r rr21`%. Note that the percentages for your overall network response rate were calculated using your network roster data from the RAND foundation. `r if(rr22<50){"Given your network response rate was under 50%, we urge you to interpret these results with caution as they represent only a portion of your network’s membership. Please reach out to Partners for Network Improvement (PNI) for tips on how to increase response rates moving forward."}`
...
rolegroup_table<-datasummary(net_role +1 ~ N, data=network_data_2022, output="flextable")
rolegroup_table%>%
  set_header_labels(net_role="Network role", N="Number of respondents")%>%
  width(width=3.25)
```
Note that these response rates includes ALL survey respondents from your network, including those who did not complete all survey questions.

Table 2 describes the **distribution of respondents by school**:\
<INSERT SCHOOL TABLE HERE.
...
``` {r know_leaders, echo=FALSE, fig.id="knowLeaders", fig.cap=paste ("Network members who know who networks leaders are (n=", know_leaders_data$Total, "respondents)")}
know_leaders_plot<-ggplot(tab_know_leaders, aes(x= Prop, y= fct_rev(factor(nl_background_1))))+
  geom_col(fill="dodgerblue2")+
  scale_y_discrete(labels = scales::wrap_format(25))+
  scale_x_continuous(breaks=extended_breaks(6), labels = scales::percent_format(accuracy = 1L))+
  labs (
    title="I know who the leaders are",
    y="",
...
`r if(params$leaders_engage){"Figure # displays **how network members report typically engaging with network leaders**. Note, these data are only for those members who indicated they know who their leaders are (see Figure #)."}`
...
``` {r group_communication, echo=FALSE, eval=params$leaders_networkcomms, fig.height=4, fig.id="gcom", fig.cap=paste("Amount of Communication with Network Sources (n=", groupcom_data$Total, "respondents)")}
gcom_plot<-tab_gcom%>%
...
```{asis time_text, echo=params$nre_intensity}
Figure \@ref(fig:time) displays **how much time network members estimate they spend participating in the network weekly**.
```

``` {r participation_time, eval=params$nre_intensity, echo=FALSE, fig.id="time", fig.cap=paste ("Estimated Weekly Network Participation (n=", time_data$Total, "respondents)")}
...
```{asis expect_text, echo=params$nre_expectations}
Figure \@ref(fig:expect) displays **how much time network members estimate they spend participating in the network weekly**.
```

```{r participation_expect, eval=params$nre_expectations, echo=FALSE, fig.id="expect", fig.cap=paste("Expected Participation in Network Activities (n=", expect_data$Total, "respondents)")}
...
rec_graph<-ggplot(tab_recommend, aes(x=recommend_1, y=Freq, fill=net_role))+
...
meetings_donut <- ggplot(tab_team_meet_donut) +
    ...
    xlim(c(0, 5)) +
   scale_fill_manual(values=c("dodgerblue2", NA, NA)) +
...
```{asis state_text, echo=params$sa_state}
\newpage
# How do network members perceive the network's alignment with state piorities?
```

`r if(params$sa_state){"An important, yet often difficult, aspect of network health is aligning the networks’ goals and activities with other priorities in schools’ surrounding environments. This increases the sense of coherence within network schools and may contribute to network engagement and sustainability."}`

`r if(params$sa_state){"While the primary report provides information about how network members perceive the network’s alignment with school and local district priorities, here we present data on the alignment of network goals with state-level priorities. Figure # displays the **distribution of network members (except hub members) who agree that the work they are doing in the network is consistent with the state’s programs and policies aimed at improving the network outcome**."}`
