  ...
  officedown::rdocx_document:
    ...
    plots:
      ...
      caption:
        ...
        sep: ': '
params:
  network_name: Denver Public Schools
  engagement: TRUE
  time: FALSE
  expectations: TRUE
  equity: TRUE
...
pal2<-c("navy", "deepskyblue1")
pal3<-c("navy", "deepskyblue1", "lightblue1")
...
useful_plot<-tab_useful%>%
  ...
  scale_fill_brewer(direction=-1, breaks=c("N/A; haven't done this year", "Not at all useful", "A little bit useful", "Moderately useful", "Useful", "Very useful"))+
  theme_classic()+
  theme(plot.title.position="plot",
...
rec_graph1<-ggplot(tab_recommend, aes(x=recommend, y=Prop))+
  geom_col(fill = "deepskyblue1")+
  scale_y_continuous(labels = scales::percent_format(accuracy = 1L))+
...
rec_graph2<-ggplot(tab_recommend, aes(x=recommend, y=Prop, fill=net_role))+
...
rec_graph3<-ggplot(tab_recommend, aes(x=recommend))+
...
rec_graph4<-ggplot(tab_recommend, aes(x=recommend, group=net_role, fill=net_role))+
...
rec_graph5<-ggplot(tab_recommend, aes(x=recommend, y=Prop, fill=net_role))+
...
rec_graph6<-ggplot(tab_recommend, aes(x=recommend, y=Freq, fill=net_role))+
  geom_col(position="stack")+
  scale_x_continuous(breaks=c(2,4,6,8,10))+
  labs (
    title="Would you recommend this network to a colleague?",
...
rec_graph7<-ggplot(tab_recommend, aes(x=recommend, y=Freq, fill=net_role))+
  geom_col(position="dodge")+
  scale_x_continuous(breaks=c(2,4,6,8,10))+
  labs (
    title="Would you recommend this network to a colleague?",
...
Figure \@ref(fig:teamDonut) displays **the percentage of members (excluding the network hub) who indicated they are part of a local improvement team**.  

```{r team_meetings_donut, echo=FALSE, fig.id="teamDonut", fig.height=2, fig.cap=paste ("Percentage of Members who Report Participating in Local Improvement Team Meetings (n=", team_meet$Total, "respondents)")}

meetings_donut <- ggplot(tab_team_meet) +
