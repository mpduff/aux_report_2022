...
```{r participation_expect, eval=params$expectations, echo=FALSE, fig.id="expect", fig.cap="Expected participation in network activities"}
...
``` {r useful_activities, echo=FALSE, fig.height=4, fig.id="useful", fig.cap="Usefulness of various network activities"}

useful_plot<-ggplot(tab_useful, aes(x=fct_reorder(names_util, Pct), y=Prop, fill=values_util, position="stacked"))+
  geom_col()+
  coord_flip()+
  ...
  scale_fill_brewer(breaks=c("Very useful", "Useful", "Moderately useful", "A little bit useful",
                             "Not at all useful", "N/A; haven't done this year"))+
  theme_classic()+
  theme(plot.title.position="plot",
