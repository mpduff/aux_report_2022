...
knitr::opts_chunk$set(warning = FALSE, message = FALSE, dpi=600, tab.topcaption = TRUE, fig.topcaption = TRUE, fig.width=6, fig.height=3.5, fig.showtext=TRUE) 
```
...
specific_network_data_2021<-specific_network_data_2021%>%
  mutate("gender_v3"=fct_explicit_na(gender_v3, "No response"))
specific_network_data_2021$gender_v3
tab_gender<-specific_network_data_2021%>%
  group_by(gender_v3)%>%
  summarize(Freq=n())%>%
  mutate(Prop=Freq/sum(Freq))%>%
  ungroup()
...
specific_network_data_2021<-specific_network_data_2021%>%
  mutate("so_part_expec"=fct_explicit_na(so_part_expec, "No response"))
specific_network_data_2021$so_part_expec
tab_expect<-specific_network_data_2021%>%
...
```{r prepare_util_stackedbars}
specific_network_data_2021$so_part_util_012021<-factor(specific_network_data_2021$so_part_util_012021, 
 levels=c(1,2,3,4,5,9),
 labels=c("Not at all useful", "A little bit useful", "Moderately useful", "Useful", "Very useful", "N/A or I haven't done this activity/nin the network this school year"))
specific_network_data_2021<-specific_network_data_2021%>%
  mutate("so_part_util_012021"=fct_explicit_na(so_part_util_012021, "No response"))
specific_network_data_2021$so_part_util_012021

specific_network_data_2021$so_part_util_022021<-factor(specific_network_data_2021$so_part_util_022021, 
 levels=c(1,2,3,4,5,9),
 labels=c("Not at all useful", "A little bit useful", "Moderately useful", "Useful", "Very useful", "N/A or I haven't done this activity/nin the network this school year"))
specific_network_data_2021<-specific_network_data_2021%>%
  mutate("so_part_util_022021"=fct_explicit_na(so_part_util_022021, "No response"))
specific_network_data_2021$so_part_util_022021

specific_network_data_2021$so_part_util_152021<-factor(specific_network_data_2021$so_part_util_152021, 
 levels=c(1,2,3,4,5,9),
 labels=c("Not at all useful", "A little bit useful", "Moderately useful", "Useful", "Very useful", "N/A or I haven't done this activity/nin the network this school year"))
specific_network_data_2021<-specific_network_data_2021%>%
  mutate("so_part_util_152021"=fct_explicit_na(so_part_util_152021, "No response"))
specific_network_data_2021
...
```{r response_rate_by_race_ethnicity, echo=FALSE, fig.id="rrRace", fig.cap="Survey Respondents by Racial Ethnic Background"}
...
```{r response_rate_by_gender, echo=FALSE, fig.id="rrGender", fig.cap="Survey Respondents by Gender"}
gender<-ggplot(tab_gender, aes(x=gender_v3, y=Prop)) +
  geom_col(fill="deepskyblue1") +
  coord_flip()+
  scale_y_continuous(labels=scales::percent)+
  labs (
...
``` {r participation_time, eval=params$time, echo=FALSE, fig.id="time", fig.cap="Estimated weekly network participation"}
