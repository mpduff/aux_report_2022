...
```{r label_race_ethnicity_factors}
...
tab_expect<-expect_data%>%
  ...
  mutate(Pct=(percent(Prop, accuracy=1)))
```
#Trying to work on the viz of time committed and time expectation
...
meetings_donut<-meetings_donut+facet_grid(~net_role)
meetings_donut

meetings_stack<-ggplot(tab_team_meet, aes(x=part_team_meetings_1, y=Prop, fill=net_role))+
  geom_col(position=position_stack())+
  coord_flip()+
  labs (
    title="Participation",
